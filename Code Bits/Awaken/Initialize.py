...
# it is the number of seconds to wait per each 0.1 degree of movement
...
"""while Position < Desired_Position:
    ####INVERSE KINEMATICS MATH HERE#####
    Corrected_Array = Angle_Array * Correction_Array
    for i in Corrected_Array:
            while Angle_Array < Desired_Angle_Array:
                Angle = Angle + 0.1
                Signal = ((1000 * Angle) / 90) + 1500
                pi.set_servo_pulsewidth(i, Signal)
                if Angle_Array >= Desired_Angle_Array:
                    break
                if Angle_Array >= Max_Angle_Array:
                    break
                if Angle_Array <= Min_Angle_Array:
                    break
                time.sleep(Speed)
"""
